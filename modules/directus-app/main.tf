...
resource "aws_s3_bucket" "cms-us-east-1" {
  provider = "aws.us-east-1"
  ...
  replication_configuration {
    ...
    rules {
      id     = "${var.r53_subdomain}-us-west-2-replicate"
      status = "Enabled"

      destination {
        bucket = "${aws_s3_bucket.cms-us-west-2.arn}"
      }
    }
  }

  policy = "${data.aws_iam_policy_document.cms-us-east-1-bucket-policy.json}"
...
resource "aws_s3_bucket" "cms-us-west-2" {
  ...
  policy = "${data.aws_iam_policy_document.cms-us-west-2-bucket-policy.json}"
...
resource "aws_cloudfront_distribution" "cms" {
  ...
  origin {
    domain_name = "${aws_s3_bucket.cms-us-east-1.bucket_regional_domain_name}"
  ...
  origin {
    domain_name = "${aws_s3_bucket.cms-us-west-2.bucket_regional_domain_name}"
...
resource "aws_route53_record" "cms4" {
  provider = "aws.us-east-1"
...
resource "aws_route53_record" "cms6" {
  provider = "aws.us-east-1"
...
resource "aws_iam_role" "replication" {
  ...
  name = "${lower(var.prefix)}-bucket-replication-role"
...
resource "aws_iam_policy" "replication" {
  ...
  name = "${lower(var.prefix)}-bucket-replication-policy"
...
data "aws_iam_policy_document" "replication-policy" {
  ...
  statement {
    ...
    resources = [
      "${aws_s3_bucket.cms-us-east-1.arn}",
    ]
  }
  ...
  statement {
    ...
    resources = [
      "${aws_s3_bucket.cms-us-west-2.arn}/*",
    ]
  }
}

data "aws_iam_policy_document" "cms-us-east-1-bucket-policy" {
  provider = "aws.us-east-1"
...
data "aws_iam_policy_document" "cms-us-west-2-bucket-policy" {
  provider = "aws.us-west-2"
